...
class ESIMCopilot:
    ...
    def process_query(self, query: str, image_path: Optional[str] = None) -> str:
        ...
        visual_query = any(k in query.lower() for k in [
            "diagram", "screen", "what's on", "current view", "circuit shown", "schematic", "layout", "visible", "labels"
        ])

        if image_path is None and visual_query:
            image_path = self._take_screenshot()
            if image_path:
                query = f"{query} (based on screen diagram)"

        if image_path and os.path.exists(image_path):
            image_info = self._analyze_image(image_path, query)
            if any(k in query.lower() for k in ["text", "read", "label", "extract", "ocr"]):
