...
class ESIMCopilot:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.text_data_dir = config.get("text_data_dir", "src/knowledge_base")

        self.device = config.get("device", "cpu")

        self.screenshot_dir = config.get("screenshot_dir", "src/screenshots")
    ...
    def process_query(self, query: str, image_path: Optional[str] = None) -> str:
        ...
        if any(keyword in query.lower() for keyword in screen_keywords) and image_path is None:
        ...
        # Construct conditional prompt sections
        visual_section = f"\n\nVisual Analysis:\n{image_info}" if image_info else ""
        ocr_section = f"\n\nOCR Text:\n{ocr_info}" if ocr_info else ""
        rag_section = f"\n\nKnowledge Base Context:\n{rag_info}" if rag_info and "No relevant" not in rag_info else ""
...
Only use the provided documents and extracted information. Do not make up any details, give the answer in 40 words, in point-wise manner, with proper detailing, make sure to give the answer quick.
...
{query}{visual_section}{ocr_section}{rag_section}
...
"""
        ...
        try:
            ...
            # Optional: Remove excessive repetition
            if response_text.count("Esim") > 5:
...
#     # Handle screenshot auto-capture if applicable
#     screen_keywords = ["screen", "diagram", "current view", "what's on the screen", "what you see"]
#     if any(keyword in query.lower() for keyword in screen_keywords) and image_path is None:
#         image_path = self._take_screenshot()
#         if image_path:
...
#     # Analyze image & extract OCR text
#     if image_path and os.path.exists(image_path):
#         image_info = self._analyze_image(image_path, query)
#         if any(k in query.lower() for k in ["text", "read", "label", "extract"]):
#             ocr_info = self._ocr_image(image_path)
...
# User Question:
# {query}
# {visual_section}{ocr_section}{rag_section}
...
#         # Optional de-duplication of repetitive content
#         if response_text.count("Esim") > 5:
#             lines = response_text.split("\n")
#             unique_lines = list(dict.fromkeys(lines))
#             response_text = "\n".join(unique_lines[:12])
...
def process_query_stream(self, query: str, image_path: Optional[str] = None):
    image_info, ocr_info = "", ""
...
#     def run(self):
#         try:
#             response = self.copilot.process_query(self.query, self.image_path)
#         except Exception as e:
#             response = f"[Worker Error] {e}"
#         self.response_signal.emit(response)


class OllamaWorker(QThread):
    response_signal = pyqtSignal(str)  # For final complete message
    stream_signal = pyqtSignal(str)    # For live updates
